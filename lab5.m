%% filter design
clc;
%clear all;
S=5;
beta=1/3;
M1=4;
M2=8;
M=100;
Num2=0;
Den2=0;
Num1=0;
Den1=0;
fc=350e6;

wc=2*pi*fc;
g1=rcosdesign(beta,S,M1,'sqrt')/sqrt(M1);
G1=fft(g1,1000);
g2=rcosdesign(beta,S,M2,'sqrt')/sqrt(M2);
%impz(G1);


% signal x1 generation
fs=30e6;
L1=2^13;
L2=2^12;
A1=1;
A2=1;
Q = 64;
w1=-pi/3;
h1=M1*g1;
x1 = randi([0 Q-1],1,L1);
y1 = qammod(x1,Q,'UnitAveragePower',true);
y1=upsample(y1,M1);% upsample
y12=A1*conv(y1,h1);
l1=length(y12);
n1=(0:l1-1);
a1=exp(1i*n1*w1);
y12=y12.*a1;


% signal x2 generation
%signal x2
w2=pi/6;
x2 = randi([0 Q-1],1,L2);
h2=M2*g2;
y2 = qammod(x2,Q,'UnitAveragePower',true);
y2=upsample(y2,M2);
y21=A2*conv(y2,h2);
l2=length(y21);
n2=(0:l2-1);
a2=exp(1i*n2*w2);
y5=y21.*a2;
append=zeros(1,length(y5)-length(y12));
y12=[y12 append];
ytx=y5+y12;    
%% passband generation

ytxn=upsample(ytx,M);
len=length(ytxn);
glp=rcosdesign(beta,S,M,'sqrt')/sqrt(M);
hlp=M*glp;
ytxnhlp=conv(ytxn,hlp);
m=1:length(ytxnhlp);
b1=sqrt(2)*exp(1i*wc/(M*fs)*m);
ytxn2=ytxnhlp.*b1;
 yc=real(ytxn2);
%% channel
  cn=exp(0.1j*pi)*[1 zeros(1,15) 2.4 zeros(1,15) 1]/4; 
 % freqz(cn)
   yrc=conv(yc,cn,'same');
 SNR=5;
%yrc=yc;
yrc=awgn(yrc,SNR);

%% Demodulation
m1=0:length(yrc)-1;
 b2=sqrt(2)*exp(-1i*wc/(M*fs)*m);
 yrxn=yrc.*b2;
 yrxlp=conv(yrxn,glp);
yrx=downsample(yrxlp,M);
%% delay compensation

yrxd=yrx(S+1:end);
%% Equalization  

for Neq=1:10
 [heq, dmin,Err]=equalize1(ytxn,yrxd,Neq);
 yrxeq=conv(yrxd,heq);
yrxeq=yrxeq(dmin+1:end);
%% detection of x2(n)
%ytx=ytx(S+1:end);
 %demod 
 j2=0:length(yrxeq)-1;
  a9=exp(-1i*j2*w2);
 yrx2=yrxeq.*a9;
 yrec12=conv(yrx2,g2);
 yrec22=downsample(yrec12,M2);
 yrec32=yrec22/A2;
 xest2=qamdemod(yrec32,Q,'UnitAveragePower',true);
 Num2=sum(abs(x2).^2);
 Den2=sum(abs(xest2(1:L2)-x2).^2);
  SIDR2(Neq)=10*log10(Num2/Den2);
  Num2=0;
  Den2=0;
 % scatterplot(yrec32);
 % title('xest2');
%% Detection of signal x1(n)
j1=0:length(yrxeq)-1;
  a6=exp(-1i*j1*w1);
 yrx1=yrxeq.*a6;
 yrec12=conv(yrx1,g1);
 yrec22=downsample(yrec12,M1);
 yrec31=yrec22/A2;
 xest1=qamdemod(yrec31,Q,'UnitAveragePower',true);
 Num2=sum(abs(x1).^2);
 Den2=sum(abs(xest1(1:L1)-x1).^2);
 
  SIDR1(Neq)=10*log10(Num2/Den2);
end
%SIDR11(SNR,:)=SIDR1;
scatterplot(yrec32);
scatterplot(yrec31);
%title('xest1');
% subplot(211);
% plot(SIDR1);
% xlabel('Neq: Order of the equalizer');
% ylabel('SIDR(dB)');
% title('SIDR1');
% subplot(212);
% plot(SIDR2);
% xlabel('Neq: Order of the equalizer');
% ylabel('SIDR(dB)');
% title('SIDR2');