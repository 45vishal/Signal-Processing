...
% ylabel('Magnitude response of the signal');
    %%
    len=1:length(yrc);
w0th=5*10e-7*pi;
 alpha=0.1*pi;
 yrc=yrc.*exp(-1i*(w0th*len+alpha));
%% Band pass filter
...
a=real(yrx)./max(abs(real(yrx))); 
b=imag(yrx)./max(abs(imag(yrx)));
B=10;
aq=quant(a,B);
bq=quant(b,B);
aq=aq*max(abs(real(yrx)));
bq=bq*max(abs(real(yrx)));
yrx=aq+i*bq;
yrxd=yrx(S+1:end);
%% linearization
